...
`current_resolution_action_valid=false`. v185 then repaired that blocker as
diagnostics-only work by strict-filtering those `7` invalid rows without
backfill, leaving `137` rows, `24` branches, `6` seeds, and `9` forced actions.
...
without a separate explicit task. v188 then ran diagnostics/support evidence
only after validating the pinned v187 exact digest and required route. It
searched `carrion_only@120` seeds `13,19,29,37,41,43` with `6` branch points
and `30` bounded policy-visible script continuations. Aggregate attempted
continuations had terminal survivors by seed
`{13: 3, 19: 3, 29: 4, 37: 3, 41: 5, 43: 4}` but also `285` unsupported
resolved actions, so aggregate attempts are not support evidence. The selected
positive support evidence is one legal replay-verified branch continuation for
seed `13`, branch `carrion-only-seed-13-branch-0-tick-0-agent-9`, continuation
script `conserve_after_carrion`, terminal alive `1`, births `5`, unsupported
requested actions `0`, and unsupported resolved actions `0`. It wrote
...
`output/mind/v188-terminal-carrion-survival-support-trajectories/`. It
recommended exactly
...
resolved actions, and closed training/runtime/promotion lifecycle flags. It did
not rerun branch exploration or generate support. All `860` unsupported
resolved records were observation-valid movement requests that became invalid
under the live resolution mask and resolved to `stay`, with legality reason
`not_in_resolution_action_mask`. Root cause:
`action_mask_timing_mismatch_same_tick_movement_occupancy_race`. v191 wrote
`output/mind/mind-v3-v191-carrion-survivor-continuation-legal-support-repair-architecture-review.json`
with exact digest
`eef890ac70140028f9d407d0b24fca40827947e75153ba5e9563b027a4233ce5`, classified
as
...
and recommended exactly `v192_action_resolution_contract_repair_no_training`.
Lifecycle flags stayed closed: no training, no training artifact, no slice-3
...
lower-specificity-only evidence dominates and grafting/imputation remains unsafe
and non-authorizing, v201 selected
...
gaps, found `0/6` clean legal support under the cap, and recommended v191.
...
- v184: diagnostics-only v178-style audit of the canonical v183 expanded
  ...
  reported `0` differences and `2` matching files.
- v185: diagnostics-only repair and re-audit for the v183 target-resolution
  blocker. Repair command:
  `npm run sim:mind:v3:carrion-survivor-continuation-v185-v183-target-resolution-repair`.
  The repair validated the v184 exact digest
  `ad9a43a670c4fce5c4ceb7a3ce54abfbc153f3d0545a3762d78c65d55d3302aa`, the
  canonical v183 source report digest
  `7281380512c4a3ce9eb0951ce8f6a1b132a74b78f7fcaafe6af3adf2bb5d16da`, and
  the canonical v183 dataset digest
  `e83424b8bb6e00a03e2afbbabd4d62c71dedfa0dec3beb482bdc73c2de1a81ef`.
  It strict-filtered the `7` invalid attack-resolution rows without backfill,
  leaving `137` rows, `24` branches, `6` seeds, and `9` forced actions. Repair
  report:
  `output/mind/mind-v3-v185-carrion-survivor-continuation-v183-target-resolution-repair.json`,
  exact digest
  `327586651948e13670cf15285472a934c8cd0a8fc9784d2dcdb092d102c46486`.
  Repaired dataset:
  ...
  Repaired audit command:
  `npm run sim:mind:v3:carrion-survivor-continuation-v185-repaired-transition-row-dataset-audit -- --expected-v185-report-exact-digest 327586651948e13670cf15285472a934c8cd0a8fc9784d2dcdb092d102c46486 --expected-dataset-digest 532817eb68cebf34cfb27f8abbbd86631cb142e03127661286d88d5154320f51`.
  The repaired audit passed source, schema, key/value leakage, feature
  contract, identity, action-mask, observation, target, and default support
  checks. Its target audit had `0` failures and its route is
  `v186_transition_row_policy_training_slice_2_opt_in`; it is not first-slice
  authorization and it did not start training. Repaired audit report:
  ...
  Lifecycle flags stayed closed across v185: `training_ran=false`,
  `training_artifact_created=false`, `runtime_artifact_created=false`,
  `runtime_action_selection_changed=false`, `promotion_authorized=false`, and
  `slice_2_training_consumed=false`. The v185 report, repaired dataset, and
  repaired audit report are durable after
...
- v187: diagnostics-only v186-delta blocker review. Command:
  ...
  explicit task.
- v188: diagnostics/support evidence only for terminal carrion survival.
  Command:
  `npm run sim:mind:v3:carrion-survivor-continuation-v188-terminal-carrion-survival-support`.
  The command validated the v187 report exact digest
  `4366edf8b1b53e9974d6548e5fe9876e305693711a1b5d66192ef42f9b6228c6` and route
  `v188_terminal_carrion_survival_support_generation_or_feasibility_proof_no_training`
  before searching `carrion_only@120` seeds `13,19,29,37,41,43`. Search budget:
  `6` branch points, `30` bounded policy-visible script continuations, horizon
  `120`, with replay verification. Aggregate attempted continuations had
  terminal survivors by seed `{13: 3, 19: 3, 29: 4, 37: 3, 41: 5, 43: 4}` but
  included `285` unsupported resolved actions, so those aggregate attempts are
  not support evidence. The selected support evidence is one legal deterministic
  replay-verified trajectory: seed `13`, branch
  `carrion-only-seed-13-branch-0-tick-0-agent-9`, continuation
  `conserve_after_carrion`, terminal alive `1`, births `5`, unsupported
  requested actions `0`, and unsupported resolved actions `0`. Report:
  `output/mind/mind-v3-v188-carrion-survivor-continuation-terminal-survival-support.json`,
  exact digest
...
- v191: diagnostics-only targeted legal-support repair / architecture review.
  Command:
  `npm run sim:mind:v3:carrion-survivor-continuation-v191-legal-support-repair-architecture-review`.
  The command validated the pinned v190 report exact digest
  `b53de9d19f67f681e334520c030978f97f6f7486e44a9f9808be11de269234b5`,
  required route
  `v191_targeted_legal_support_repair_or_architecture_review_no_training`,
  v190 backup metadata, `18` branch points, `90` replay-verified
  continuations, `0/6` clean support, `0` unsupported requested actions,
  `860` unsupported resolved actions, and closed training/runtime/promotion
  lifecycle flags. It did not rerun branch exploration or generate support; it
  parsed the canonical v190 report plus trajectory files. All `860`
  unsupported resolved records were movement requests that were valid in the
  tick-start public observation mask, invalid in the live resolution mask,
  resolved to `stay`, and carried legality reason
  `not_in_resolution_action_mask`. v191 classifies the root cause as
  `action_mask_timing_mismatch_same_tick_movement_occupancy_race`: a
  counterfactual script/action-resolution contract mismatch caused by
  deterministic same-tick action ordering, not a v190 digest/count reporting
  bug, illegal requested action, water/hazard issue, or runtime policy change.
  Unsupported resolved counts by seed were `{13: 127, 19: 149, 29: 194,
  37: 117, 41: 133, 43: 140}`. Near-clean seed `29` needs action-diversity cap
  repair only (`stay` share `0.5154`, unsupported requested/resolved `0/0`);
  near-clean seed `41` needs action-resolution contract repair plus a small
  cap repair (`stay` share `0.5014`, unsupported resolved `4`). Report:
  `output/mind/mind-v3-v191-carrion-survivor-continuation-legal-support-repair-architecture-review.json`;
  exact digest:
  `eef890ac70140028f9d407d0b24fca40827947e75153ba5e9563b027a4233ce5`.
  Classification:
  ...
  `runtime_artifact_created=false`, `runtime_action_selection_changed=false`,
  `promotion_authorized=false`, and `gate_relaxation_allowed=false`. The v191
  report is durable after
  `/Users/njm/evolution-sim-p0-backups/20260613T122723Z-v191-legal-support-repair-architecture-review.tar.zst`
  and
  `gdrive:evolution-sim-backups/archives/20260613T122723Z-v191-legal-support-repair-architecture-review.tar.zst`,
  archive SHA256
  ...
  `rclone check /Users/njm/evolution-sim-p0-backups
  gdrive:evolution-sim-backups/archives --include
  "20260613T122723Z-v191-legal-support-repair-architecture-review.tar.zst*" --one-way`
  reported `0` differences and `2` matching files. The campaign budget remains
...
- `2026-06-14` v201 high-specificity action-complete source contract audit:
  no-training command
  `npm run sim:mind:v3:carrion-survivor-continuation-v201-high-specificity-action-complete-source-contract-audit`.
  v201 directly validated the durable v200/v199/v198 exact digests and
  validated v197 through v198's source-pin validation. It audited the v199
  candidate-key rows against the frozen v195 artifact utility table without
  replay, support generation, dataset mutation, or training. The audit
  confirmed `2714` missing current-valid action demands across present
  high-specific keys, same high-specific action support count `0`, same
  high-specific support-floor failures `0`, and lower-specificity-only support
  for `2362` demands (`0.870302` share). Missing actions absent at both the same
  high-specific key and known lower-specificity keys accounted for `352`
  demands. Lower-specificity grafting/imputation remains unsafe and
  non-authorizing. v201 wrote
  `output/mind/mind-v3-v201-carrion-survivor-continuation-high-specificity-action-complete-source-contract-audit.json`;
  exact digest:
  `ed401b7a4e5ef5ae572040d6a9973e23b51951f2bd273e48797c094132af94d2`.
  Classification:
  ...
  start or consumption, no runtime artifact, no runtime integration, no runtime
  policy or action-selection change, no gate relaxation, no support generation
  or expansion, no dataset mutation, and no promotion. The v201 report is
  durable after
  `/Users/njm/evolution-sim-p0-backups/20260614T205238Z-v201-high-specificity-action-complete-source-contract-audit.tar.zst`
  and
  `gdrive:evolution-sim-backups/archives/20260614T205238Z-v201-high-specificity-action-complete-source-contract-audit.tar.zst`,
  archive SHA256
