...
- 2026-04-28: Added `reproduction.animal_mode_energy_requirement_multiplier` with validation and applied it to non-`none` meat modes in the reproduction energy requirement helper. This original implementation improved animal-mode parent births in the 20-seed ecology bank without treating it as a complete fix for animal-resource access. 2026-05-09 audit note: this static role bonus was later found to be too broad; current code gates animal-mode reproduction-resource bonuses by recent animal-energy share.
...
- 2026-05-01: Continued reducing `world.py` only through behavior-preserving boundaries. Tick orchestration now delegates to `runtime/ticks.py`, and summary end-surface construction delegates to `runtime/reporting.py`; `SimulationWorld` keeps thin compatibility entry points. The slice also locks signal decay semantics: active multi-tick signal fields remain historical emitted substrate, not live source-state beacons, so decayed fields retain original source id/location/tick without creating new emission events.
- 2026-05-01: Prepared the rare multi-offspring strategy as a gated observable contract slice. `ReproductionConfig` now has disabled-by-default multi-offspring controls, `fecundity_potential` can unlock the capability only when explicitly enabled, and reproductive readiness stats expose `multi_offspring` counts. Added real fixture worlds for Stage 2/3 role imbalance so gates warn on expression scarcity and incompatible mate-search constraints without default-run drift.
- 2026-05-01 tick/summary extraction and gated multi-offspring validation: compileall for `python/evolution_sim` and `python/tests`, focused extraction/signal/multi-offspring/gate fixture regressions, `npm run sim:test` (210 tests in 122.186s), refreshed and verified replay goldens, `npm run sim:gate:quick` (pass, no blockers or warnings), `npm run sim:bench:quick` (`summary_seed7_ticks20` median wall `0.6188s`, median RSS `92976 KiB`), and `npm run sim:gate:release -- --output output/evaluations/foundation-release-current.json` passed with no blockers or warnings in `881.2436s`. Refreshed golden hashes: `seed7_ticks20=4f07f16365f38e31cd1a66eac749991eb2dd5a808f59e08cf0e36ae7a8b9c91e`, `seed7_ticks100=f7d53db75334fc1e20983bc98aef0ef80aa2850d5c602005ebf7837fb6ffb008`, `speciation_seed_ticks320=d5990e7f5d3a12e27d4ae03b2e5b8227fd2c8e5bca313429703d199a843f044f`, and `seed5_ticks120_provenance=f35fcf709ec389fe2ad34f742d8b0e8a73427e3490b3775cad76ce32c65743bf`.
...
- 2026-05-09: A reproduction-to-learning audit found a hidden pair-accounting bug. With `cooldown_ticks=0`, the same sexual pair could produce reciprocal same-tick births, masking the separate issue that trajectory reproduction credit was derived only from primary-parent `tick_birth_pairs`. The runtime now tracks tick-level reproduction participants separately from taxonomy birth pairs: `tick_birth_pairs` remains one primary-parent row per child, while `tick_reproduction_parent_ids` and `tick_reproduction_parent_child_groups` lock current-tick sexual participants and credit both biological parents in trajectory rewards.
- 2026-05-09 reproduction accounting validation: the new pair-lock and both-parent trajectory-credit tests failed before the fix and pass after it; focused reproduction and trajectory contract suites passed, `python3 -m compileall -q python/evolution_sim python/tests` passed, `npm run sim:test` passed with 469 tests and 25 skipped, and `npm run sim:golden` verified `seed7_ticks20=65fbf6115aa268c39e03de69441d2578d43abda1711a89a2f8d9099ac60a9966`, `seed7_ticks100=f9a7f6429a4a374422610bd397f5bd019221f5b8c4a5fd3a2edeb18049e1c6ad`, `speciation_seed_ticks320=0b2ccc5ebf1b349cfdc65a29abb5bb42cfcd6574942fb6d56499dda37709294c`, and `seed5_ticks120_provenance=7845c1212b37c06d2b8721f489d0e17bad9732b2e37aeac0eedfd9ab1c568abe`.
- 2026-05-09: A follow-on persistence/credit audit found three additional Mind v3 hardening bugs that did not break the default simulator suite. First, `collect_trajectory --mind-v3-autonomous-evolution --include-policy-update-trace` wrote trajectories that `load_trajectory_jsonl()` rejected because the loader only accepted the older online-bandit trace schema. Second, once v3 traces were accepted, the torch IQL online-feedback helper would have treated v3 controller-update traces as online-bandit feedback because it only checked `action` and `reward_signal`. Third, `MindV3EvolutionPolicy.observe_transition()` updated controllers from passive trajectory rows, crediting `stay` for agents killed before their turn.
- 2026-05-09 follow-on audit repairs: the dataset loader now validates both online-bandit and Mind v3 update-trace schemas; torch online-feedback extraction ignores non-online-bandit traces; Mind v3 ignores passive and foreign-policy transition feedback; and `load_mind_v3_controller_metadata()` now rejects malformed weights, malformed biases, and unsupported controller architectures instead of silently defaulting bad payloads to zero/current behavior. Focused regressions, a real `collect_trajectory`/load probe, full `python.tests.test_mind_v1`, `npm run sim:test` with 473 tests and 25 skipped, `npm run sim:golden:quick` with `seed7_ticks20=65fbf6115aa268c39e03de69441d2578d43abda1711a89a2f8d9099ac60a9966` and `seed7_ticks100=f9a7f6429a4a374422610bd397f5bd019221f5b8c4a5fd3a2edeb18049e1c6ad`, and `git diff --check` pass after the repairs.
- 2026-05-09: A turn-order and observation-boundary audit confirmed one more hidden ecology bias. Runtime action resolution and reproduction parent priority were both fixed-ID ordered, so lower `agent_id` agents had a permanent first claim on contested movement, attacks, mate selection, and reproduction space. The tick loop now uses deterministic `seed_tick_agent_hash_permutation_v1` ordering, exposes the current `world.tick_action_order`, and routes reproduction through the same order when reproduction runs inside a tick. A 200-tick 20-agent priority probe gave every agent at least one first slot (`distinct_first=20`) with mean rank range `9.71..11.28`.
- 2026-05-09 observation-boundary follow-up: the current trainable paths do not use observation metadata as a feature. The encoded observation and feature-policy keys ignore `metadata` and raw in-observation `action_mask`; v3 uses `metadata.agent_id` only as runtime controller-addressing state and uses the explicit action mask only for legality. The remaining unresolved design question is the intended sensor model for `NAVIGATION_RADIUS = 10`, not an accidental metadata leak.
- 2026-05-09: A follow-up ecology audit found two more hidden distortions after fixed-ID priority was removed. First, animal-mode reproduction threshold, parent cost, and child starting-resource bonuses were static meat-mode bonuses, so a mixed or carnivore lineage could receive animal-resource reproduction advantages after living almost entirely on plants. These bonuses now scale from the parent's recent animal-energy share: plant-only recent diet gets no animal-resource bonus, and the configured multiplier saturates once recent animal intake reaches 20% of recent food energy.
- 2026-05-09: The same audit found that the observation heuristic had a non-hunting fallback attack branch. Attack-capable mixed omnivores could attack adjacent agents even when fed and standing on rich plant food, producing hidden predation pressure in a plant-arena fixture. That fallback was removed; explicit hunter/matched-diet attack paths remain. A hand probe of the plant-only fixture changed from mixed omnivore `8` kills and `25` births versus herbivore `22` births before the fix to mixed omnivore `0` kills and `21` births versus herbivore `26` births after it. The shocky seasonal fixture still gives mixed omnivores limited reproduction when they actually consume carcass/fresh-kill energy: `3` mixed births with `6.45%` animal diet share in the focused probe.
- 2026-05-09 ecology-audit validation: focused action/reproduction/headless fixture tests passed, `python3 -m compileall -q python/evolution_sim python/tests` passed, `npm run sim:test` passed with 479 tests and 25 skipped, `npm run sim:golden` verified `seed7_ticks20=a03527f0e1f74c43c498129c7b172caa983f34720d48a90bd077ba0b923d9a0d`, `seed7_ticks100=bf55e23d802efe1a252f40f87d0abc2c710340ffeb613edf14942d2e12208f63`, `speciation_seed_ticks320=5e3342d0c7a9e5550cd14216ba3ffbe6e8b33b6563ef597aa15995e6519ff6b6`, and `seed5_ticks120_provenance=63c6a6ff83da29824991de328143de3e1bbcf609a68d845ae9dc5e813714dc4b`; `npm run sim:gate:quick -- --fail-on-blockers` passed with no blockers or warnings, `npm run sim:bench:quick` passed with `summary_seed7_ticks20` median wall `0.6192s`, and `git diff --check` was clean.
- 2026-05-09 deeper ecology-gate audit: the post-fair-order ecology gate failure was not a valid proof that mixed controllers ignored all animal-resource opportunities. Two hidden accounting problems were found. First, animal-resource presence and reachability counted trace residue above `1e-9`, so runs could report policy-actionable carrion/fresh-kill while the observation navigation rounded the signal to zero. Second, the gate divided consuming runs by all alive runs, including runs with no reachable animal-resource opportunity. The unsupported adjacent hunter/mixed "eat blocked neighboring resource" experiment was removed rather than kept as a policy cheat. Opportunity accounting now uses `ANIMAL_RESOURCE_OPPORTUNITY_MIN_ENERGY = 0.02`, and gate run-share floors use `animal_resource_opportunity_runs` (`consumption_events > 0` or `reachable_ticks > 0`) instead of raw `alive_runs`.
- 2026-05-09 deeper ecology-gate validation: the 20-seed probe now reports mixed alive-share `6/14` but meaningful opportunity-share `6/9`; the ecology gate passes in `66.5529s` with no blockers or warnings. The generated rollup shows hunter `20/20`, mixed `6/9`, and scavenger `13/18` animal-resource consuming/opportunity runs. This does not erase the remaining issue: mixed still has `3` reachable-unconsumed opportunity runs, so contested-resource/pathing and reproduction pressure remain real next targets.
- 2026-05-09 deeper ecology-gate validation commands: `python3 -m compileall -q python/evolution_sim python/tests`, focused opportunity/gate/action tests, `npm run sim:test` (`482` tests, `25` skipped), `npm run sim:golden`, `npm run sim:gate:quick -- --fail-on-blockers`, `npm run sim:gate:ecology -- --fail-on-blockers`, `npm run sim:bench:quick`, `jq empty output/evaluations/foundation-ecology-current.json`, and `git diff --check` passed. Updated golden hashes: `seed7_ticks20=a03527f0e1f74c43c498129c7b172caa983f34720d48a90bd077ba0b923d9a0d`, `seed7_ticks100=863d0f65b4c1c6fdc58281b9376925dbdb4e31e564d5223767a2bd0e97b0e613`, `speciation_seed_ticks320=7ca08a55d228e5626e4e96433a13563fe98e446c43bb3d5531527e105c3dbf61`, and `seed5_ticks120_provenance=63c6a6ff83da29824991de328143de3e1bbcf609a68d845ae9dc5e813714dc4b`.
- 2026-05-10 opportunity-timing audit: opportunity presence now uses the decision-time resource snapshot rather than end-of-tick deposits, so same-tick generated fresh-kill/carcass no longer inflates policy-visible opportunity. A golden mismatch exposed one follow-up bug: `*_consumed_ticks` had been tied to `*_present_ticks`, losing same-tick generated-and-consumed resources. Consumption tick counters now derive from consumption events independently, while present/unconsumed counters remain decision-time opportunity metrics. Validation: focused opportunity regressions, `python3 -m compileall -q python/evolution_sim python/tests`, `npm run sim:test` (`486` tests, `25` skipped), `npm run sim:golden`, `npm run sim:gate:quick -- --fail-on-blockers`, `npm run sim:bench:quick`, JSON validation for generated ecology/v3 reports, and `git diff --check` passed. Updated golden hashes: `seed7_ticks20=a27958bc604a70a9471e8fc5db261e754b6d7ead748e2ef34d7ca3d5654d8cad`, `seed7_ticks100=56bf1357a8d6c339f0e36a4a30fc8b3edcf46c1e14cc30e6d6361356b794ada2`, `speciation_seed_ticks320=bef894d6896c6a751250f48250002c13f6c11704bc5e26273ced3389ccfc71c0`, and `seed5_ticks120_provenance=79974db44b3dc3b527b2824430c0d2904b55c3b986f449c9e5b417634b2992ee`.
...
2026-05-09 follow-up: the raw statement above is incomplete after the v3 observation work. `mind_observation_v3` now also exposes scalar directional navigation targets with `NAVIGATION_RADIUS = 10`, while the trainable tensor excludes `metadata` and raw in-observation `action_mask`. The current trainable paths use metadata only for runtime controller lookup and use the explicit action mask for legality/masking. The remaining design question is not a hidden metadata leak; it is whether a radius-10 directional target is the intended sensory model for autonomous ecology or whether it is too much built-in navigation.
...
2026-05-09 follow-up: action and reproduction priority now use `seed_tick_agent_hash_permutation_v1`. The permutation is deterministic under fixed seed/tick, does not consume the world RNG stream, and is exposed as `world.tick_action_order` for audit/debug checks. Lifecycle death cleanup still uses sorted order because it does not allocate contested resources or mates.

Options:

- current: use a seeded hash-permutation order per tick for actions and reproduction;
- future: move to a two-phase simultaneous action model with explicit conflict resolution if collision semantics become the next bottleneck.
...
6. Remove sorted-ID action priority as a hidden policy feature:
   - implemented 2026-05-09 with `seed_tick_agent_hash_permutation_v1` for action and reproduction priority;
   - keep simultaneous action selection and explicit conflict resolution as a future larger semantic slice if pairwise collision artifacts remain.
...
As of 2026-05-01, the Foundation has moved beyond inert scaffolding: live reproductive groups, same-group facultative sexed recombination, reproductive-readiness signals, config-aware signal/action contracts, opt-in opaque communication emissions, viewer replay validation, reproductive/tick runtime boundary cleanup, gated rare multi-offspring behavior, and the first proto-role/X/Y/Z expression scaffold are implemented and covered by focused checks.

The active blocker is now semantic completion rather than scaffolding. Mind v1 should still wait until full Z group-imbalance behavior/costs, hybridization policy, and learned-state inheritance semantics are either implemented or explicitly deferred with stable contracts.
