...
class MindV3CarrionSurvivorContinuationV175V174RouteCorrectionAuditTests(
    unittest.TestCase
):
    def test_npm_entrypoint_exists(self) -> None:
        ...
        self.assertEqual(
            package["scripts"][
                "sim:mind:v3:carrion-survivor-continuation-v175-v174-route-correction-audit"
            ],
            (
                "PYTHONHASHSEED=0 PYTHONPATH=python python3 -m "
                "evolution_sim.cli."
                "mind_v3_carrion_survivor_continuation_v175_v174_route_correction_audit"
            ),
        )

    def test_v174_digest_mismatch_closes_invalid_without_lanes(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            report = run_carrion_survivor_continuation_v175_v174_route_correction_audit(
                ...
                output_path=paths["v175_report"],
                v176_plan_output_path=paths["v176_plan"],
                expected_v174_exact_digest="wrong",
        ...
        self.assertIn(
            "v174_unexpected_exact_digest",
            report["source_validation"]["failures"],
        )
        self.assertEqual(
            report["classification"]["primary"],
            "v175_invalid_closed_no_shadow",
        )
        self.assertTrue(
            report["lanes"]["lane_a_per_seed_lane_c_recompute"]["skipped"]
        )
    ...
    def test_synthetic_route_survives_strict_per_seed_controls(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            paths, payloads = _write_inputs(tmpdir)
        ...
        lane_a = report["lanes"]["lane_a_per_seed_lane_c_recompute"]
        self.assertTrue(lane_a["floor_passed"])
        self.assertEqual(lane_a["seed_41"], {})
        lane_b = report["lanes"]["lane_b_clean_negative_controls"]
        self.assertTrue(lane_b["floor_passed"])
        self.assertFalse(lane_b["clean_controls_erase_margin"])
        self.assertTrue(lane_b["clean_controls_never_pull_from_heldout_seed"])
        self.assertEqual(
            report["classification"]["primary"],
            "v175_action_ranking_fit_still_worth_diagnostics_only",
        )
        self.assertFalse(report["v176_plan_output"]["written"])
    ...
    def test_cli_writes_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            written = json.loads(paths["v175_report"].read_text(encoding="utf-8"))
    ...
    v174_report = _report(
        {
            ...
            "classification": {
                "primary": EXPECTED_V174_CLASSIFICATION,
                "labels": [EXPECTED_V174_CLASSIFICATION],
            },
            "source_validation": {
                "passed": True,
                "observed_v173_exact_digest": v173_report["exact_digest"],
                "observed_v172_dataset_digest": stable_payload_digest(rows),
            },
            "lanes": {
                "lane_c_action_ranking_capacity_probe": {
                    "best_ranking_entry": {"label": "all_train", "k": None}
                }
            },
            "dataset_digest": stable_payload_digest(rows),
            **_lifecycle_flags(),
