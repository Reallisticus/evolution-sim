...
from evolution_sim.io.source_manifest import (
    SourceManifestError,
    source_file_hash_manifest as _source_file_hash_manifest,
)
...
def source_file_hash_manifest(repository_root: str | Path) -> dict[str, object]:
    try:
        return _source_file_hash_manifest(repository_root)
    except SourceManifestError as error:
        raise RecurrentScaleCampaignError(str(error)) from error
