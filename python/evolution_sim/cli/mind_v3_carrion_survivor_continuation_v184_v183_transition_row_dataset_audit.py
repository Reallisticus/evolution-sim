...
from evolution_sim.mind.carrion_survivor_continuation_v184_v183_transition_row_dataset_audit import (
    DEFAULT_OUTPUT_PATH,
    DEFAULT_V183_REPORT_PATH,
    DEFAULT_V183_TRANSITION_DATASET_PATH,
    run_carrion_survivor_continuation_v184_v183_transition_row_dataset_audit,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run diagnostics-only v184, a fresh v178-style audit of the "
            "canonical v183 expanded compact transition-row dataset. The command "
            "may authorize only a future explicit slice-2 opt-in training route; "
            "it does not train, create runtime artifacts, change runtime action "
            "selection, relax gates, or promote."
        )
    )
    parser.add_argument(
        "--v183-report",
        ...
        dest="v183_report",
        type=Path,
        default=DEFAULT_V183_REPORT_PATH,
    )
    parser.add_argument(
        "--transition-dataset",
        type=Path,
    ...
    parser.add_argument(
        ...
        dest="expected_v183_report_exact_digest",
        default=None,
        help="Defaults to the canonical v183 exact digest pinned by the contract.",
    )
    parser.add_argument(
        "--expected-dataset-digest",
        default=None,
        help="Defaults to the canonical v183 expanded transition-row dataset digest.",
    )
...
def main() -> None:
    ...
    try:
        report = run_carrion_survivor_continuation_v184_v183_transition_row_dataset_audit(
            v183_report_path=args.v183_report,
        ...
        raise SystemExit(
            "failed to run v184 carrion survivor-continuation v183 transition-row "
            f"dataset audit: {exc}"
        ) from exc
...
def _print_summary(report: Mapping[str, object], output: Path) -> None:
    ...
    print(f"carrion_survivor_continuation_v184_v183_transition_row_dataset_audit={output}")
